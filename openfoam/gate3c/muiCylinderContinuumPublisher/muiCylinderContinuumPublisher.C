...
int main(int argc, char *argv[])
{
    ...
    (
        "Publish a converged full-cylinder continuum snapshot to Gate 3C DSMC"
    );
    ...
    std::vector<gate3c::State> states;
    states.reserve(gate3c::angularCells);
    ...
    for (int pointIndex = 0; pointIndex < gate3c::angularCells; ++pointIndex)
    {
        ...
        (
            mesh,
            gate3c::continuumSamplePoint(pointIndex)
        );
        if (celli < 0)
        {
            Foam::Info<< "GATE3C_FAIL role=publisher reason=no_sample_cell"
        ...
        if (!state.physical())
        {
            Foam::Info<< "GATE3C_FAIL role=publisher reason=nonphysical_state"
                      << " point=" << pointIndex << Foam::endl;
            return 2;
        }
    ...
    Foam::Info<< "GATE3C_PUBLISHER_SNAPSHOT time=" << runTime.timeName()
              << " points=" << states.size()
    ...
    mui::uniface3d interface("mpi://continuum/gate3c");
    for (int couplingStep = 1;
         couplingStep <= gate3c::kineticSteps;
         ++couplingStep)
    {
        for (int pointIndex = 0; pointIndex < gate3c::angularCells; ++pointIndex)
        {
        ...
        const double acknowledgement =
            gate3c::fetchAcknowledgement(interface, couplingStep);
        if
        (
        ...
        {
            Foam::Info<< "GATE3C_FAIL role=publisher reason=acknowledgement"
                      << " step=" << couplingStep
    ...
    Foam::Info<< "GATE3C_PASS role=publisher steps="
              << gate3c::kineticSteps

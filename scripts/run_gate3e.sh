...
BUILD_DIR=${BUILD_DIR:-"$ROOT/build/gate3e"}
RUN_ID=${SLURM_JOB_ID:-manual-$(date -u +%Y%m%dT%H%M%SZ)}
RUN_DIR=${RUN_DIR:-"$ROOT/run/gate3e-$RUN_ID"}
if [[ -e "$RUN_DIR" ]]; then
    printf 'ERROR: refusing to overwrite Gate 3E run directory: %s\n' \
        "$RUN_DIR" >&2
    exit 2
fi
...
trap '
    status=$?
    if (( status != 0 )); then
        printf "GATE3E_PIPELINE_FAIL stage=%s status=%d\n" "$STAGE" "$status" >&2
    fi
' EXIT
rm -f "$REPORT_DIR/gate3e_summary.json" "$REPORT_DIR/gate3e_live.log"

# shellcheck disable=SC1091
source "$ROOT/scripts/load_openfoam_if_needed.sh"
GATE3D_SUMMARY="$REPORT_DIR/gate3d_summary.json"
GATE3C_SUMMARY="$REPORT_DIR/gate3c_physical_summary.json"
GATE3C_COMPARISON="$REPORT_DIR/gate3c_wall_comparison.csv"
STAGE=prerequisite
python3 "$ROOT/scripts/require_gate3d_pass.py" "$GATE3D_SUMMARY"
...
(
    cd "$ROOT"
    python3 -m unittest -q tests.test_gate3e
)
STAGE=build
BUILD_DIR="$BUILD_DIR" bash "$ROOT/scripts/build_gate3e.sh"
...
case "$GATE3C_RUN_DIR" in
    "$ROOT"/run/gate3c-*) ;;
    *)
        printf 'ERROR: Gate 3C run directory is outside the repository.\n' >&2
        exit 2
        ;;
esac
for case_name in continuum hybrid; do
    if [[ ! -d "$GATE3C_RUN_DIR/$case_name" ]]; then
        printf 'ERROR: Gate 3C source case is missing: %s\n' \
            "$GATE3C_RUN_DIR/$case_name" >&2
        exit 2
    fi
    cp -a "$GATE3C_RUN_DIR/$case_name" "$RUN_DIR/$case_name"
done
...
foamDictionary "$DSMC_CONTROL" -entry application -set dsmcFoamGate3E
...
if [[ -z "$MPI_LAUNCHER" || ! -x "$MPI_LAUNCHER" ]]; then
    printf 'ERROR: mpirun is unavailable in the Gate 3E runner.\n' >&2
    exit 127
fi
LIVE_LOG="$REPORT_DIR/gate3e_live.log"
STAGE=live_concurrent_coupling
timeout --signal=TERM --kill-after=30 2400 \
    "$MPI_LAUNCHER" \
    -np 1 env GATE3E_COMPARISON="$GATE3C_COMPARISON" \
        "$BUILD_DIR/openfoam/rhoCentralFoamGate3E" \
        -case "$RUN_DIR/continuum" \
    : \
    -np 1 env GATE3C_ROLE=live \
        "$BUILD_DIR/openfoam/dsmcFoamGate3E" \
        -case "$RUN_DIR/hybrid" \
    2>&1 | tee "$LIVE_LOG"
grep -q 'GATE3E_PASS role=continuum_live' "$LIVE_LOG"
grep -q 'GATE3E_PASS role=dsmc_live' "$LIVE_LOG"

SUMMARY="$REPORT_DIR/gate3e_summary.json"
STAGE=analysis
python3 "$ROOT/scripts/analyze_gate3e.py" \
    --log "$LIVE_LOG" \
    --summary "$SUMMARY" \
    --run-dir "$RUN_DIR"
printf 'GATE3E_SUMMARY=%s\n' "$SUMMARY"
printf 'GATE3E_RUN_DIR=%s\n' "$RUN_DIR"
STAGE=complete

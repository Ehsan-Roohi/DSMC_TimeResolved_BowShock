...
ACTIVE_JOB=$(
    ...
    | awk -F'|' \
        '$2 == "mui-of-g3h" && ($3 == "PENDING" || $3 == "RUNNING" || $3 == "COMPLETING") {print $1; exit}'
)
if [[ -n "$ACTIVE_JOB" ]]; then
    printf 'ERROR: Gate 3H job %s is already active; refusing duplicate submission.\n' \
        "$ACTIVE_JOB" >&2
    exit 2
fi
...
if [[ ! -s "$ROOT/reports/gate3c_wall_comparison.csv" ]]; then
    printf 'ERROR: Gate 3C wall comparison is required before Gate 3H.\n' >&2
    exit 2
fi
...
JOB_ID=$(sbatch \
    --parsable \
    --chdir="$ROOT" \
    --export=ALL,GATE3H_ROOT="$ROOT" \
    "$ROOT/slurm/unity_gate3h.sbatch")
printf 'GATE3H_JOB_ID=%s\n' "$JOB_ID"
printf 'WATCH=squeue -j %s\n' "$JOB_ID"
printf 'LOG=%s/reports/gate3h-%s.out\n' "$ROOT" "$JOB_ID"

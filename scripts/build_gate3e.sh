...
for command_name in mpic++ wmake; do
    if ! command -v "$command_name" >/dev/null 2>&1; then
        printf 'ERROR: required Gate 3E build command not found: %s\n' \
            "$command_name" >&2
        exit 2
    fi
done
...
export GATE3E_BUILD_DIR="$BUILD_DIR"
...
mkdir -p "$GATE3E_BUILD_DIR/openfoam"

for target in rhoCentralFoamGate3E dsmcFoamGate3E; do
    source_dir="$ROOT/openfoam/gate3e/$target"
    wclean "$source_dir" >/dev/null 2>&1 || true
    wmake "$source_dir"
done

for executable in rhoCentralFoamGate3E dsmcFoamGate3E; do
    if [[ ! -x "$GATE3E_BUILD_DIR/openfoam/$executable" ]]; then
        printf 'ERROR: Gate 3E executable was not built: %s\n' \
            "$executable" >&2
        exit 2
    fi
done
printf 'GATE3E_CONTINUUM_EXECUTABLE=%s\n' \
    "$GATE3E_BUILD_DIR/openfoam/rhoCentralFoamGate3E"
printf 'GATE3E_DSMC_EXECUTABLE=%s\n' \
    "$GATE3E_BUILD_DIR/openfoam/dsmcFoamGate3E"

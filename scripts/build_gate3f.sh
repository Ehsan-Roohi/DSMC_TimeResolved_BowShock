...
for command_name in mpic++ wmake; do
    if ! command -v "$command_name" >/dev/null 2>&1; then
        printf 'ERROR: required Gate 3F build command not found: %s\n' \
            "$command_name" >&2
        exit 2
    fi
done
...
export GATE3F_BUILD_DIR="$BUILD_DIR"
...
mkdir -p "$GATE3F_BUILD_DIR/openfoam"

for target in rhoCentralFoamGate3F dsmcFoamGate3F; do
    source_dir="$ROOT/openfoam/gate3f/$target"
    wclean "$source_dir" >/dev/null 2>&1 || true
    wmake "$source_dir"
done

for executable in rhoCentralFoamGate3F dsmcFoamGate3F; do
    if [[ ! -x "$GATE3F_BUILD_DIR/openfoam/$executable" ]]; then
        printf 'ERROR: Gate 3F executable was not built: %s\n' \
            "$executable" >&2
        exit 2
    fi
done
printf 'GATE3F_CONTINUUM_EXECUTABLE=%s\n' \
    "$GATE3F_BUILD_DIR/openfoam/rhoCentralFoamGate3F"
printf 'GATE3F_DSMC_EXECUTABLE=%s\n' \
    "$GATE3F_BUILD_DIR/openfoam/dsmcFoamGate3F"
